...
Q1=eye(1); % Weight Matrix for States
%Q=D_Transform'*Q1*D_Transform; % Transformed Weight MAtrix for the Transformed States
%Q=10^(0)*eye(4);
Q=10^(0)*diag([10000,10,1,1]);
R=10^(1); % Weight Matrix for Input U
...
simout_est=sim('LQR_FeedBackRegulation_EstimatedPlant','StopTime',num2str(tfinal))
...
simout_Real=sim('LQR_FeedBackRegulation_RealPlant','StopTime',num2str(tfinal))
